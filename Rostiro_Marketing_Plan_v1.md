...
**Placeholder update (2026-07-06, revised same day):** all three `ProductVideoDemo` slots on the Features page are filled with real rendered `.mp4` placeholders (`public/videos/`), built with Remotion (`remotion/` directory, dev dependency). First pass rendered each feature floating in empty space with no surrounding UI — founder feedback correctly flagged this as unusable. Rebuilt with:
- **`remotion/components/AppFrame.tsx`** — a full recreation of the real authenticated shell (icon dock + System Bar, matching `components/nav/Sidebar.tsx`/`SystemBar.tsx`'s actual layout and icon set) so every clip shows the feature living inside the real product chrome, with resting/background content behind it, not an isolated fragment on a blank canvas.
- **`remotion/components/camera.ts`** — a Ken-Burns-style push-in/pull-back on the specific element each clip is demonstrating, instead of a flat static wide shot the whole time.
- **`remotion/components/Callout.tsx`** — spring-animated annotation bubbles (with a pointer arrow) that name what's happening on screen, since these are silent clips with no voiceover.
- **Kickoff transition** and **Interrupt Stack** still use the real `brandTokens.ts` colors and real `STATE_TRANSITION_MS`/`AUTO_DISMISS_MS` timing underneath all of the above — accurate to actual product behavior, captioned "recreation, real footage pending."
- **Multi-league connect** still opens on a recreation of the real onboarding screen's actual layout (centered card, step label — onboarding runs outside the authenticated shell in the real product) before cutting to the AppFrame Pulse view, with placeholder account names and a permanent on-screen "ILLUSTRATIVE REENACTMENT — NOT REAL ACCOUNT FOOTAGE" watermark (repositioned to bottom-center after the first pass placed it directly on top of the System Bar's own chips).
