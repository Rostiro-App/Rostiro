...
| T-130 | Password reset flow (added July 6, 2026, founder-reported gap: "where is the password reset link"). Confirmed by searching the whole codebase — no `resetPasswordForEmail` call, no `/reset-password` route, no "Forgot password?" link exist anywhere. Real, larger-scope build, not a quick fix: needs (1) a "Forgot password?" entry point (removed the temptation to add a dead link pointing nowhere on `app/(auth)/login/page.tsx` for this same reason), (2) a request-reset page/form calling `supabase.auth.resetPasswordForEmail(email, { redirectTo })`, and (3) a confirm page that captures Supabase's recovery session from the emailed link and lets the user set a new password via `supabase.auth.updateUser({ password })`, with real empty/error/success states throughout. Not started. |
