...
| T-124 | Animated hero banners, marketing homepage and Features page (added July 6, 2026, founder-reported). Founder flagged both `app/page.tsx`'s hero section and `app/features/page.tsx`'s hero section as static and wants them animated. Scope undefined — what specifically animates (background, headline entrance, a live embedded component like `RostiroStatesCycle`) is an open creative decision, not yet discussed. Not started. |
| T-125 | Marketing placeholder videos re-flagged as insufficient a second time (added July 6, 2026, revises the placeholder sub-scope under T-112/`Rostiro_Video_Shotlist.md`). The three Remotion-rendered `ProductVideoDemo` placeholders (kickoff transition, Interrupt Stack, multi-league connect) were rebuilt once already to add real app chrome, camera motion, and callouts after the first pass rendered features floating in empty space — but founder review against live product screenshots (`/pulse`) found the recreation still doesn't match: real nav icons (not placeholder squares), the real greeting/progress-bar format ("Good evening. / 4 decisions across 2 leagues · Est. 6 min / TODAY 0/4"), real Pulse card anatomy (platform + league tag, priority badge, Open/Done/Snooze/✕ buttons, timestamp), and the live ADP-mover ticker are all missing from the recreation. Needs a further rebuild pass once discussed with the founder; the real founder-shot B-roll this was always standing in for (`Rostiro_Video_Shotlist.md`) remains separately DEFERRED and unaffected by this. Not started. |
