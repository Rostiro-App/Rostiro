...
| Founder brainstorm: state-simulation tool + event-trigger animations | Founder wants to see/capture each Rostiro State on demand (for QA and for marketing stills/video) — logged as a scoped, not-yet-built follow-up (a dev-only force-any-state toggle extending DEMO_MODE) rather than started, since the founder chose to stay on the PRD's own Phase 3 order first. |
...
| T-135 | ✓ **Code done, blocked on DNS** (added July 6, 2026, shipped July 7, 2026, founder gap-analysis pass ahead of opening real signups). Both auth emails (signup confirmation, password reset) were rewritten to go through Resend instead of Supabase's own unbranded sender: `browserClient.auth.signUp()`/`resetPasswordForEmail()` (which always trigger Supabase's own built-in email, with no per-call template override short of Custom SMTP + dashboard-edited templates) were replaced with two new server routes, `app/api/auth/signup/route.ts` and `app/api/auth/forgot-password/route.ts`, that call `admin.auth.admin.generateLink()` (generates the confirmation/recovery link, sends nothing) and hand the link to new `lib/resend.ts`, which sends Rostiro's own branded HTML template (table-based layout for email-client compatibility, brand kit's navy/signal-blue tokens, the notification-icon mark, marketing-wordmark treatment since an email is a hero surface, not an in-product screen). Both client pages (`app/(auth)/signup/page.tsx`, `app/(auth)/forgot-password/page.tsx`) now POST to these routes instead of calling the Supabase client SDK directly; the underlying confirm/reset link mechanism is unchanged, so `/api/auth/callback` still handles it exactly as before. Verified for real: a direct Resend API call from `rostiro-brand-kit.md`'s color tokens confirmed the actual `RESEND_API_KEY` in `.env.local` is valid and the account is live, but sending `from` a `rostiro.com` address fails outright (`"The rostiro.com domain is not verified"` — Resend rejects it before it ever reaches an inbox). Sending the exact same branded template through Resend's sandbox sender (`onboarding@resend.dev`) to the account's own verified address (`lawrence@rostiro.com`) delivered successfully, confirming the code path, the API key, and the template itself all work end-to-end. What's left isn't code: `rostiro.com` needs its SPF/DKIM/DMARC records added and verified in Resend's dashboard (same DNS gap T-137 already logs) before `RESEND_FROM_EMAIL=noreply@rostiro.com` will actually send to real users. |
| T-136 | Explicit Terms/Privacy consent checkbox at signup (added July 6, 2026, founder gap-analysis pass). Today's signup (`app/(auth)/signup/page.tsx`) only shows passive text — "by creating an account, you agree to Terms" — with no affirmative action required ("browsewrap"). An explicit "I agree" checkbox ("clickwrap") is standard practice and meaningfully more enforceable before real payments start flowing. Not started. |
...
| T-138 | Error/uptime monitoring (added July 6, 2026, founder gap-analysis pass). No Sentry/APM or status page exists — a production break today is discovered from a user complaint, not a dashboard. Doesn't need to be elaborate to start (Sentry's free tier or equivalent). Not blocking to open signups, but worth having before real traffic scales up. Not started. |
...
| T-140 | Wire real ESPN draft tracking to a route (added July 6, 2026, split out from T-139). `getEspnDraftDetail` and friends already exist in `lib/espn.ts` and work when called directly (confirmed in an earlier session), but nothing wires them to `/api/draft/session` the way Sleeper/Yahoo are wired. Also needs one clean confirmation pass of `mDraftDetail`'s `picks` array populating pick-by-pick against a real, human-paced draft — the one prior test used a bot-filled ESPN mock draft, which completes near-instantly and gets torn down by ESPN immediately after, so pick-by-pick population was never directly observed (only `inProgress` flipping true at kickoff was confirmed). Not started. |
| T-141 | Notes schema + General note UI (added July 7, 2026, founder request: "almost no place for users to input their questions or add context"). New `notes` table: `id`, `user_id`, `league_id` (nullable, FK `connected_leagues`), `player_id` (nullable), `type` (`'general' \| 'ask_copilot'`), `body`, `response` (nullable, ask_copilot only), `status`, `created_at`/`updated_at`. One shared table/form for both note types rather than two separate features. This task covers General only: a contextual "Add note" entry point (Leagues page, Trades page, player card — no new nav tab, same reasoning as skipping a standalone notebook) with a type dropdown, optional league/player select, and a 500-char-capped body. General notes never call Claude at write time — pure storage. Not started. |
| T-142 | Ask Copilot note type — trade/scenario queries (added July 7, 2026, founder request, motivating example: "find me a trade for Patrick Mahomes, looking for RB or TE of similar return"). Builds on T-141's schema with `type = 'ask_copilot'`: league select is required (a trade ask is meaningless without knowing which league's real rosters to search), body capped at 280 chars to force a focused ask and keep the prompt small. Keeps this codebase's standing discipline (`app/api/trades/analyze/route.ts`, every other `generate*` in `lib/claude.ts`): deterministic layer finds real candidates first, Claude only explains them, never invents a trade partner it hasn't actually seen in that league's roster data. Needs one new deterministic step extending the Trade Analyzer's existing `adpValue` logic into a real candidate-finder: given a player + target position(s), scan that league's actual fetched rosters for comparable-value players at that position, then hand the top 1-3 real candidates to a new Claude reasoning function (same pattern as `generateTradeReasoning`). Cost controls: `checkAndIncrementUsage` (`lib/usageLimits.ts`) gets a new `'ask_copilot'` feature key, plan-gated weekly quota same as trade analysis; `checkRateLimit` (`lib/rateLimit.ts`) adds a per-user burst throttle (e.g. one ask per 30s) so quota can't be blown through scripted bursts. Not started. |
| T-143 | General notes feed the Copilot signal (added July 7, 2026, split out from T-141/T-142 — the loop-closing half of the notes feature). Right now every AI surface in this app tells the user something; none of them read anything the user wrote back. This task wires General notes into existing `generate*` calls as extra input context: a note on a rostered player feeds the waiver rundown (see the Claude-waiver-rundown discussion, July 6, 2026), a note on a league feeds Trade Analyzer reasoning. Depends on T-141 shipping first. Not started. |
| T-144 | Snooze alerts on Pulse interrupts (added July 7, 2026, founder request). `pulse_items` (`app/api/pulse/interrupts/route.ts`) already has a `status` column (`'open'`/etc) — add a `snoozed_until` timestamp and a `'snoozed'` status. A Snooze button on an interrupt card sets `status = 'snoozed'`, `snoozed_until = now + 24h`; the existing GET query extends to treat a snoozed item whose `snoozed_until` has passed as open again. No new table, no cron. Not started. |
