...
| Task | Description |
|---|---|
| T-102 | Mode-aware AI voice — thread `mode` through every route that calls `lib/claude.ts` (start/sit, trade analysis, draft recommendations, Pulse reasoning); each prompt-builder selects a tone block per PRD 3 (Focused: verdict-first, no hedging; Balanced: current default; Savant: advisory only, never directive). No schema changes — smallest, highest-leverage item in the sequence. |
| T-103 | Free/Pro usage quota enforcement — new `usage_counters` table + `lib/usageLimits.ts`'s `checkAndIncrementUsage()`, gating `/api/lineup/sleeper` and `/api/trades/analyze` at 3/week for Free before the Claude call. Plus the 1-league cap for Free at each platform's league-connection route. Closes a real, already-flagged revenue gap ("no weekly free/paid quota yet" — existing code comment on the Lineup page). |
| T-104 | Draft State / Standard State surface wiring — Draft Kit and live draft session pick up Draft State's accent/framing (6.13's opportunity-green language already specified); Leagues/Trades reflect Standard State. Needs a short design pass on what Standard State concretely changes on Leagues/Trades before implementation — under-specified in the PRD relative to how thoroughly Game Day's states were spec'd. |
| T-105 | Mode threaded into remaining surfaces — Leagues, Trades, live draft session, Command Palette. Mechanical once T-102/T-104 establish the pattern. |
| T-106 | The Interrupt Stack — new `components/InterruptStack.tsx` mounted in `AppShell`, implementing 7.1's "one persistent interrupt slot at a time" rule. Requires a data-shape decision (a `layer` column on `pulse_items` vs. a separate ledger — recommend the column). Retrofits `detectTouchdownSwings`/`detectMissionComplete` (P1/P2, transient) off the Action-layer queue; `detectLineupLockUrgency` (P0) keeps persistent treatment. |
| T-107 | Per-league waiver-cutoff config — new `connected_leagues` column(s) + onboarding/settings field (extends PRD 4 Step 4's existing "waiver system" field to also capture *when*); `computeState()` gains an optional per-league cutoff input, falling back to today's global Tue/Wed default. Prerequisite for accurate multi-league Waiver Day detection. |
| T-108 | Waiver Day State UI (T-94/T-98) + Film Room State UI (T-95) — Mission Briefing card layout with FAAB/roster-health math; Film Room recap card (doesn't exist yet) with real per-platform matchup win/loss data (Yahoo scoreboard, Sleeper matchups-by-week, ESPN unofficial matchup endpoint). Explicitly excludes snap-count/injury/Buy-Low-Sell-High content, which stays blocked on T-87's nflverse pipeline — render the card without those sections rather than fake data. |
