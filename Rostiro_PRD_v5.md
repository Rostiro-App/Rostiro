...
| Change | Rationale |
|---|---|
| 🚩 Cross-League Portfolio Intelligence spec added (design approved, not yet built) — League Favoriting, Critical alert tier, cross-league free-agent search, advisory lineup calls in Pulse, Studio registry entries | Written 2026-07-16 after Yahoo confirmed read-only-only access (no platform grants write access to Rostiro) and a competitive scan found FantasyPros' My Playbook already ships cross-league free-agent search + an auto-pilot feature Rostiro can't and shouldn't match (no write access). Locks in the founder's honesty rule — real per-league facts (deadlines) never transformed/guessed, portfolio/exposure synthesis is the differentiation layer — and designs for 2/5/9+-league power users without silently truncating a championship-deciding alert (the "handcuff behind an injured starter" case drove the new Critical tier, which bypasses Focused mode's card cap and push rate-limits). Full spec: `docs/superpowers/specs/2026-07-16-cross-league-portfolio-intelligence-design.md`. Player Intelligence Card's multi-league treatment and actual marketing copy drafts explicitly deferred to follow-on work. |
