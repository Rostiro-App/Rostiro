# ROSTIRO — Product Requirements Document v5.11
**Run Every League.**
...
## Changelog from v5.10 → v5.11

| Change | Rationale |
|---|---|
| 🚩 Cross-League Portfolio Intelligence spec revised to v2 (design approved, not yet built) — Canonical Player Identity, League Favoriting, RB-only Critical alert tier, cross-league free-agent search, Studio registry entries | Written 2026-07-16, revised same day after a second-opinion review caught real defects in v1: a favoriting-persistence bug (delete-based unfavoriting couldn't represent "favorited by default, one league explicitly unfavorited"), an IDOR in the favorites API (no league-ownership check before a service-role write), a self-contradiction on Critical push gating, an incomplete dedup design, and two places v1 itself violated the founder's own "never present a guess as a fact" rule — calling a depth-chart position a confirmed handcuff, and calling a static ADP gap a confident weekly start/sit signal. v2 fixes all of them: corrected `league_preferences` schema with an ownership check, a new Canonical Player Identity prerequisite (finishes an already-started, previously-unseeded `player_mappings` table rather than starting fresh), the Critical tier narrowed to running backs only with disclosed "next listed RB, not a confirmed replacement" copy and fingerprint-based dedup, free-agent search with an honest 5-state availability model (`rostered`/`free_agent`/`waivers`/`pending_transaction`/`unconfirmed`) and real rate-limit/debounce handling, and push gating that always respects `push_enabled` with zero exceptions. **Advisory proactive lineup pushes are removed from scope this pass** — the existing on-demand Start/Sit engine is unchanged; a proactive version needs real weekly projections (not ADP) first. Full spec: `docs/superpowers/specs/2026-07-16-cross-league-portfolio-intelligence-design.md`. Plan: `docs/superpowers/plans/2026-07-16-cross-league-portfolio-intelligence.md`. Player Intelligence Card's multi-league treatment and actual marketing copy drafts explicitly deferred to follow-on work. |

## Changelog from v5.9 → v5.10

| Change | Rationale |
|---|---|
| T-164 added and shipped — Studio Push Moment pack | Adds a "Push" surface to the Simulation Studio rendering an authorable iOS lock-screen push notification (9:16, 16:9-safe) as a third special render path alongside game_day/live — the reusable capture surface for the T-163 scratch-alert push and any push moment. |

## Changelog from v5.8 → v5.9

| Change | Rationale |
|---|---|
| 🚩 T-163 added — **PRIORITY, pre-Week-1** (code complete; migration + live-verify pending) — Starter Scratch Alerts | Full trace (July 11) of the injury-news pipeline found the highest-stakes gap: a starter ruled OUT ~90 min before kickoff, healthy that morning, never reaches the user through Rostiro (structured `injury_status` refreshes once/day; the 15-min ESPN RSS pipe is prose-only, never pushes; `lineup_lock` reads the stale cache). Design spec approved: extend the news cron with a deterministic scratch classifier → a fresh `player_scratches` signal driving both a reversal-safe fingerprint Pulse card and a Pro-gated, cross-league-collapsed push (starters + high-confidence only). Surfaced and folded in two latent issues: the global `push_enabled` toggle is never actually enforced in `pushToUser`, and `engagement_log.trigger_type`'s CHECK constraint must be altered for the new trigger. Spec: `docs/superpowers/specs/2026-07-11-starter-scratch-alerts-design.md`. |
...
| `STRIPE_SECRET_KEY` / `STRIPE_WEBHOOK_SECRET` / `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY` | Billing (live) | Stripe dashboard → Developers → API keys / Webhooks |
...
- T-85 Pricing rebuild (Stripe) — Free / Rostiro Pro ($9.99/mo) / Founder Season Pass ($59) / Founding 500 ($149 lifetime, capped at 500). ✓ **Done — Stripe is fully live** (July 12, 2026). Real account created, live keys in place, all four tiers processing real payments end-to-end; schema, plan-gating logic, and webhook handling verified against live Stripe. No longer a blocker — pricing, Founding 500, and "capped at 500" scarcity content are all cleared to post.
...
| T-85 | ✓ **Done — Stripe fully live** (July 12, 2026). Pricing rebuild — Free / Pro / Founder Season Pass / Founding 500 in Stripe, onboarding trial copy (Step 2), Start/Sit and Trade Analyzer tier gating from Starter→Pro (6.4/6.5). Real account, live keys, all four tiers processing real payments end-to-end; webhook + plan-gating verified live. Was the last Phase-1 launch blocker; now closed. |
...
| T-163 | 🚩 **PRIORITY — pre-Week-1. Code complete; migration + live-verify pending** (added July 11, 2026; design spec at `docs/superpowers/specs/2026-07-11-starter-scratch-alerts-design.md`). **Starter Scratch Alerts** — closes the highest-stakes gap in the news pipeline, traced in full July 11: a starter ruled OUT ~90 min before kickoff (healthy that morning) currently *never reaches the user through Rostiro* — `injury_status` refreshes once/day (~5am ET from Sleeper's 5MB player payload), and the only fast pipeline (ESPN RSS, every 15 min) is prose-only (never updates status, never pushes, only a passive `player_news` card on next Pulse open). The one game-day injury path (`lineup_lock`) reads that same stale cache, so a fresh Out on a previously-healthy starter slips through entirely; the game-day trigger layer is also Sleeper-only. **The fix ("Option D"):** extend the 15-min news cron with a deterministic scratch classifier (`lib/scratchClassifier.ts`, ESPN language → Sleeper Out/Doubtful/Questionable vocab) writing a fresh per-player signal (`player_scratches`) that drives **both** a reversal-safe in-app Pulse card (via the existing `injury:{lg}:{player}:{status}` fingerprint in `syncPulseItems`) **and** a Pro-gated, cross-league-collapsed push (new `detectStarterScratches` mirroring `detectTouchdownSwings`' `byUser` grouping + `engagement_log` one-shot dedup). **Locked decisions:** signal+both-outputs; no batching window (fire fast — the one-player-N-leagues multiplier collapses in a single tick); deterministic classifier (no Claude in the cron); **starters only, high-confidence only pushes** (medium-confidence is card-only — a push can't be unsent, so the confidence gate is load-bearing); **Pro + Sleeper only** (free tier still gets the card). **UX layer:** one push per user naming all affected leagues ("Josh Allen — ruled out. Starting in Lawrence's Legends +3 others"), a "why you got this" league line, and a user preference (`users.notify_scratches`, default on) surfaced at onboarding + settings. **Surfaced two latent issues** documented in the spec: (1) the global `push_enabled` toggle is stored/shown but never enforced in `pushToUser` — this fixes it for *all* triggers; (2) `engagement_log.trigger_type` has a hardcoded CHECK constraint that must be altered to add `'starter_scratch'` or every `claimTrigger` insert 23514-throws. **Per-user dedup confirmed safe** (composite `unique (user_id, trigger_type, dedupe_key)` — not a global key). **Explicitly out of scope / separate future track:** a faster *structured* injury feed (gap-analysis Options A/B); `player_scratches` is deliberately the seam a lower-latency source would later write into. **Honesty guardrail:** ~15-min, headline-derived, high-confidence only — claim "we ping you within minutes of the news breaking," never "instant/real-time." Effort: Low–Med, pre-Week-1 feasible (~70% reuse). Implementation plan next. **Status (July 11, 2026):** implemented across 10 tasks — deterministic classifier, `player_scratches` signal, `detectStarterScratches` detector, cron wiring, Pulse card merge, global `push_enabled` enforcement, and the `notify_scratches` preference with onboarding + settings toggles — all type-checked, linted, and unit-tested. `supabase/migration_scratch_alerts.sql` must still be applied to Supabase as a deploy step (not auto-applied); not yet live-verified in a preseason dev env (no real scratch + Pro user + push subscription available to observe end-to-end), same posture as T-162/T-148. |
| T-164 | ✓ **Done** (added and shipped July 11, 2026). **Studio Push Moment pack** — a new "Push" surface in the Simulation Studio (`app/demo/studio`) rendering an authorable, full-bleed iOS lock-screen push notification (9:16-native, centered on a neutral backdrop in 16:9), added as a **third special render path** (`state === 'push'`) alongside the existing `game_day`/`live` special-casing — deliberately NOT a `StatePack` (a lock-screen has no `DemoShell` app chrome). One generic `PushMoment` content model (`app/demo/lib/pushMoment.ts`) authors any push type (touchdown/lineup-lock/scratch) identically, with real-data prefill (real fixture player + real `DEMO_LEAGUES` names) and full editorial override via `PushAuthorForm`. This is the reusable **capture surface for T-163's scratch-alert push** and any future push moment; the clip is an editorial mockup carrying no user-facing claim. Route-isolated (nothing under `app/demo/**` imports product/DB/live-API modules, ESLint-enforced). Registry-generalization (folding this into the pack registry) logged as future cleanup. Manual browser click-through of `/demo/studio` → Push not yet performed; covered by unit tests (PushLockScreen render, Studio/Canvas/Panel wiring). |
| T-165 | 🚩 **MUST GET TO — founder-flagged, 2026-07-14. Not started.** **Business cost tracking, overall.** Need one real, current picture of what Rostiro actually costs to run — not just Claude/Anthropic API spend (already scoped: `rostiro-cockpit`'s `get_infra_costs` tool, real data via Anthropic's Usage & Cost Admin API, day/week/month, blocked on creating a separate Admin API key in Console) but the full stack: Fly.io (cockpit hosting, no public billing API found — dashboard-only), Supabase, Vercel, n8n, Postiz ($29/mo flat, already known), and any other paid tool in the marketing stack (Screen Studio, Submagic, Feedly, etc. — see `Rostiro_Marketing_Toolstack_v1.md` §12 for the last known cost table). Supabase/Vercel/n8n's actual plans/tiers and whether they expose real usage-cost APIs were deliberately not researched yet — founder chose to ship the two confirmed-automatable sources (Claude, Postiz) first rather than block the cockpit tool on full coverage. This item is the follow-up: get the Admin key created, wire it in, and either find/build real cost visibility for the rest of the stack or confirm definitively that manual dashboard checks are the ceiling for some of them. |

---

*Rostiro PRD v5.9 — July 2026*
