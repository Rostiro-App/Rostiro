...
| T-135 | Branded transactional email delivery (added July 6, 2026, founder gap-analysis pass ahead of opening real signups). Confirmed via full codebase search: `resend` is a `package.json` dependency but is called nowhere in the code — every email today (signup confirmation, and password reset once T-130 ships) goes out through Supabase's own default sender, unbranded, not from `rostiro.com`, and rate-limited to a handful of emails per hour on the free tier. Needs real Resend wiring (or Supabase custom SMTP pointed at Resend) + actual branded templates, built alongside T-130 rather than after it, since password reset emails are exactly the kind of thing that needs to not look like spam. Also needs real DNS records (SPF/DKIM/DMARC) configured for `rostiro.com` in Resend's dashboard — code wiring alone doesn't fix deliverability. Not started. |
