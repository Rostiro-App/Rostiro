...
| Task | Description |
|---|---|
| T-72 | Boot sequence + coach marks — first-login boot moment (skippable, never repeats), `<Hint>` registry component, `seen_hints` persistence, contextual first-use hints, replay from Settings/palette. **Open question (2026-07-04):** the founder wants a *mandatory* tour — a series of tooltips over the front-end UI that must be stepped through and dismissed before they disappear, not the skippable-boot-moment version originally spec'd here. Real UX-philosophy fork (skippable vs. forced walkthrough), not just an implementation detail — needs a decision before building, not silently resolved either way. Not started. |
...
| T-104 | ✓ **Done.** Draft State / Standard State surface wiring — Draft Kit and live draft session pick up Draft State's accent (matches shipped `STATE_CONFIG` amber, not 6.13's text, which says "opportunity green" and was never reconciled — flagged, not silently picked); Leagues sorts health factors by urgency during Standard State. Trades and the ticker deliberately untouched — nothing genuine to differentiate. |
