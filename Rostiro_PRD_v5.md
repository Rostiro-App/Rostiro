...
| Task | Description |
|---|---|
| T-72 | ◐ **Boot sequence done, July 4 2026** — `components/BootSequence.tsx`, ~2.2s, reuses `PulseMark` (T-91, already built anticipating "'hero' matches marketing/boot-sequence scale"). Deliberate spec change confirmed with the founder: **every login**, not first-login-only/never-repeats as originally written here — gated by `sessionStorage` instead of a per-day key, cleared on the login page so a repeat login in the same tab still replays it. State-colored: renders in Standard's blue immediately, swaps to the real active state if it resolves in time. Two real bugs caught before landing: a hydration mismatch from reading `sessionStorage` inside a `useState` initializer, and the new CSP (T-76) missing `'unsafe-eval'`, which broke React's own dev-mode tooling (confirmed via a live console error) — fixed dev-only so the deployed CSP never weakens. **Coach marks half still not started** — the mandatory-vs-skippable tour open question from before is unchanged and still needs a decision. |
