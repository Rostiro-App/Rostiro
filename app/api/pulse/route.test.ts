...
describe('GET /api/pulse — P3-8B: platform-neutral user-facing consumer', () => {
  beforeEach(() => {
    vi.resetModules()
  })
    ...
    vi.doMock('@/lib/pulse', () => ({
      buildPulseItemsForUser: vi.fn(() =>
        Promise.resolve({
          ...
          leagueCount: 2,
          coverage: [
            { connectedLeagueId: 'cl-sleeper', leagueName: 'Sleeper League', platform: 'sleeper', status: 'included_fresh', reason: null },
            { connectedLeagueId: 'cl-espn', leagueName: 'ESPN League', platform: 'espn', status: 'included_fresh', reason: null },
          ],
        })
      ),
  ...
  it('PROOF (P3-8B): coverage metadata is returned even when leagueCount is 0 — an unavailable league never looks identical to "no leagues connected"', async () => {
    vi.doMock('@/lib/supabase', () => ({
      createSSRClient: () => Promise.resolve(mockSSRClient()),
      createAdminClient: vi.fn(() => ({})),
    }))
    vi.doMock('@/lib/pulse', () => ({
      buildPulseItemsForUser: vi.fn(() =>
        Promise.resolve({
          items: [],
          leagueCount: 0,
          coverage: [{ connectedLeagueId: 'cl-espn', leagueName: 'ESPN League', platform: 'espn', status: 'unavailable', reason: 'No successful roster snapshot has been captured yet' }],
        })
      ),
    ...
    expect(body.coverage[0].status).toBe('unavailable')
  })
